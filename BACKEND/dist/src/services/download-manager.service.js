...
class DownloadManager extends events_1.EventEmitter {
     ...
     */
    async startDownload(modelId, repoId, userId, token) {
        try {
            // Check if model exists in database
            let dbModel = await (0, connection_1.query)('SELECT id FROM models WHERE repo_id = $1', [repoId]);
            let dbModelId;
            if (dbModel.rows.length === 0) {
                ...
                const insertResult = await (0, connection_1.query)(`INSERT INTO models (name, type, repo_id, status, metadata)
           VALUES ($1, $2, $3, $4, $5)
           RETURNING id`, [
                    modelInfo.id,
                ...
                await (0, connection_1.query)('UPDATE models SET status = $1 WHERE id = $2', ['downloading', dbModelId]);
            }
            // Create download job
            const downloadResult = await (0, connection_1.query)(`INSERT INTO download_queue (model_id, user_id, status, created_at)
         VALUES ($1, $2, 'pending', NOW())
         RETURNING id`, [dbModelId, userId]);
     ...
     */
    async processDownload(downloadId, repoId, modelId, token) {
        try {
            // Mark as downloading
            this.activeDownloads.set(downloadId, true);
            await (0, connection_1.query)(`UPDATE download_queue 
         SET status = 'downloading', started_at = NOW()
         WHERE id = $1`, [downloadId]);
            // Prepare destination directory
            const destDir = path_1.default.join(process.cwd(), 'models', repoId.replace('/', '_'));
            logger_1.logger.info({
            ...
            await huggingface_service_1.hfService.downloadModel(repoId, destDir, token, async (file, downloaded, total, fileIndex, fileCount) => {
                ...
                const bytesProgress = totalBytesTotal > 0
                    ...
                    : 0;
                const overallProgress = Math.floor(((fileProgress * 0.5) + (bytesProgress * 0.5)) * 100);
             ...
             SET progress = $1, 
                 ...
                 bytes_total = $3, 
                 current_file = $4
             WHERE id = $5`, [overallProgress, downloaded, total, file, downloadId]);
                // Emit progress event
                this.emit('progress', {
                    downloadId,
         ...
         WHERE id = $3`, [destDir, fileSizeMB, modelId]);
         ...
         SET status = 'failed', 
             error_message = $1, 
             completed_at = NOW()
         WHERE id = $2`, [error.message, downloadId]);
            await (0, connection_1.query)(`UPDATE models 
    ...
    async getUserDownloads(userId) {
        try {
         ...
         JOIN models m ON dq.model_id = m.id
         WHERE dq.user_id = $1
         ORDER BY dq.created_at DESC`, [userId]);
            return result.rows;
        }
        catch (error) {
            logger_1.logger.error({
    ...
    async cancelDownload(downloadId) {
        try {
            const result = await (0, connection_1.query)(`UPDATE download_queue 
         SET status = 'failed', 
