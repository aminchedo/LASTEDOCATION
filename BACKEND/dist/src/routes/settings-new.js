...
router.get('/', async (req, res) => {
    try {
       ...
       WHERE user_id = $1`, [userId]);
        ...
        logger_1.logger.error({ msg: 'get_settings_failed', error: error.message });
        res.status(500).json({
            success: false,
            error: error.message
        });
...
router.post('/', async (req, res) => {
    try {
        ...
        // Upsert settings
        await (0, connection_1.query)(`INSERT INTO user_settings (
        user_id,
        huggingface_token,
        huggingface_api_url,
        auto_download,
      ...
      DO UPDATE SET
        ...
        logger_1.logger.error({ msg: 'save_settings_failed', error: error.message });
        res.status(500).json({
            success: false,
            error: error.message
        });
 ...
 */
router.put('/huggingface/token', async (req, res) => {
    try {
        const userId = req.user?.id || 'default';
        const { token } = req.body;
        if (!token) {
        ...
        // Save token
        await (0, connection_1.query)(`INSERT INTO user_settings (user_id, huggingface_token)
       VALUES ($1, $2)
       ON CONFLICT (user_id) 
       DO UPDATE SET
         huggingface_token = $2,
         updated_at = CURRENT_TIMESTAMP`, [userId, token]);
        logger_1.logger.info({
        ...
        logger_1.logger.error({ msg: 'update_token_failed', error: error.message });
        res.status(500).json({
            success: false,
            error: error.message
        });
 ...
 */
router.delete('/huggingface/token', async (req, res) => {
    try {
        const userId = req.user?.id || 'default';
        await (0, connection_1.query)(`UPDATE user_settings
       ...
       WHERE user_id = $1`, [userId]);
        ...
        logger_1.logger.error({ msg: 'delete_token_failed', error: error.message });
        res.status(500).json({
            success: false,
            error: error.message
        });
...
//# sourceMappingURL=settings-new.js.map