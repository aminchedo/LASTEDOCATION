...
class CPUTrainer {
    ...
    async start(config, callbacks) {
        ...
        this.trainingLoop().catch(error => {
            logger_1.logger.error({ error: error.message }, 'Training loop error');
    ...
    async saveCheckpoint(tag) {
        ...
        // Save checkpoint
        await this.storage.saveCheckpoint(currentRun.id, checkpointId, { weights: this.weights, biases: this.biases }, resumeToken, tag, metric);
        // Update state
        await this.state.createCheckpoint(currentRun.id, checkpointId, tag);
        ...
        logger_1.logger.info({ checkpointId, tag, metric }, 'Checkpoint saved');
    ...
    async loadCheckpoint(checkpointId) {
        ...
        logger_1.logger.info({ checkpointId }, 'Checkpoint loaded');
    }
    ...
    async trainingLoop() {
        ...
        try {
            while (this.isTraining && currentEpoch < this.currentConfig.totalEpochs) {
                ...
                // Record metrics
                await this.metrics.recordMetric(currentRun.id, currentEpoch, currentStep, loss, undefined, // valLoss
                accuracy);
                // Callbacks
    ...
    backwardPass(_predictions, _targets) {
