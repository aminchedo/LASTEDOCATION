...
export class TrainingSSE {
  ...
  constructor(state: TrainingStateManager, metrics: MetricsService, trainer: CPUTrainer) {
    this.state = state;
  ...
  addClient(clientId: string, response: Response, lastEventId?: string): void {
    ...
    response.on('error', (error) => {
      logger.warn({ clientId, error: error.message }, 'SSE client error');
      this.removeClient(clientId);
    });
  }
  ...
  async sendStatusUpdate(clientId?: string): Promise<void> {
    try {
  ...
  async sendMetricsUpdate(clientId?: string): Promise<void> {
    try {
      const currentRun = this.state.getCurrentRun();
      if (!currentRun) {
        return;
      ...
      logger.error({ error: error.message }, 'Failed to send metrics update');
    }
  }

  async sendCheckpointUpdate(checkpointId: string, tag: string, metric: number): Promise<void> {
    try {
      const checkpoint = {
        type: 'checkpoint',
        data: {
      ...
      logger.error({ error: error.message }, 'Failed to send checkpoint update');
    }
  }
  ...
  private sendToClient(clientId: string, event: any): void {
    ...
    try {
      ...
      logger.warn({ clientId, error: error.message }, 'Failed to send SSE event to client');
      this.removeClient(clientId);
    }
  }
  ...
  private broadcast(event: any): void {
    ...
    for (const [clientId, client] of this.clients) {
      try {
        ...
        logger.warn({ clientId, error: error.message }, 'Failed to broadcast SSE event');
        this.removeClient(clientId);
      }
    }
  }
