...
async function startRealTraining(runId, config) {
    return new Promise((resolve, reject) => {
        try {
            ...
            const scriptPath = path_1.default.join(process.cwd(), 'scripts', 'train_real_pytorch.py');
            // Prepare arguments for the real training script
            const args = [
                scriptPath,
                '--model-name', config.modelName || 'HooshvareLab/bert-fa-base-uncased',
                '--dataset-path', config.datasetPath || path_1.default.join(process.cwd(), 'combined.jsonl'),
                '--output-dir', path_1.default.join(process.cwd(), 'models', `trained_${runId}`),
                '--epochs', config.epochs.toString(),
                '--batch-size', config.batchSize.toString(),
                '--learning-rate', config.learningRate.toString(),
                '--run-id', runId
            ];
            logger_1.logger.info(`Starting real PyTorch training with args: ${JSON.stringify(args)}`);
