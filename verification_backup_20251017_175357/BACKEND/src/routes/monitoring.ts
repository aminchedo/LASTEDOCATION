...
async function getRealDiskUsage(): Promise<any> {
  try {
...
async function getActiveProcesses(): Promise<number> {
  try {
    if (process.platform === 'win32') {
    ...
    logger.warn(`Failed to get process count: ${String((error as any)?.message || error)}`);
    return 0;
  }
}
...
router.get('/metrics', async (_req: Request, res: Response): Promise<void> => {
  try {
    ...
    const msg = `Error getting metrics: ${String((error as any)?.message || error)}`;
    logger.error(msg);
